...
public class BlockESPScreen extends Screen {
    ...
    public void render(DrawContext context, int mx, int my, float delta) {
        ...
        context.drawCenteredTextWithShadow(textRenderer,
                Text.literal("§lBlockESP  §r§7— Block List"),
                px + W / 2, py + (HEADER_H - 8) / 2, 0xFFFFFF);

        // ── Hint ──────────────────────────────────────────────────────────────
        context.drawCenteredTextWithShadow(textRenderer,
                Text.literal("§8Press G in-game to reopen"),
                px + W / 2, py + HEADER_H + PAD + 24, 0x555555);

        // ── Separator ─────────────────────────────────────────────────────────
        context.fill(px + PAD, listY - 6, px + W - PAD, listY - 5, 0x33FFFFFF);

        // ── Block count label ─────────────────────────────────────────────────
        context.drawTextWithShadow(textRenderer,
                Text.literal("§7Saved Blocks  §8(" + module.getTargetBlocks().size() + ")"),
        ...
        if (blocks.isEmpty()) {
            context.drawCenteredTextWithShadow(textRenderer,
                    Text.literal("§8No blocks added yet."),
                    px + W / 2, listY + 28, 0x555555);
            context.drawCenteredTextWithShadow(textRenderer,
                    Text.literal("§7Type a block name above and press Enter."),
        ...
        for (int i = start; i < end; i++) {
            ...
            // Row background
            context.fill(px + PAD, ry, px + W - PAD, ry + ROW_H - 2,
            ...
            // Remove [✕] button
            int bx = px + W - PAD - 19;
            context.fill(bx, ry + 1, bx + 17, ry + ROW_H - 3,
                    xHover ? BTN_HOVER : BTN_NORMAL);
            context.drawCenteredTextWithShadow(textRenderer,
                    Text.literal("§c✕"), bx + 8, ry + (ROW_H - 8) / 2, 0xFF5555);
        }
        ...
        if (blocks.size() > VISIBLE) {
            ...
            context.fill(px + W - 5, listY, px + W - 2, listY + trackH, 0x22FFFFFF);
            context.fill(px + W - 5, thumbY, px + W - 2, thumbY + thumbH, ACCENT);
        }
        ...
        // ── Border ────────────────────────────────────────────────────────────
        context.fill(px,         py,         px + W,     py + 1,     ACCENT);
        context.fill(px,         py + H - 1, px + W,     py + H,     ACCENT);
        context.fill(px,         py,         px + 1,     py + H,     ACCENT);
        context.fill(px + W - 1, py,         px + W,     py + H,     ACCENT);
    ...
    public boolean mouseClicked(double mx, double my, int btn) {
        ...
        for (int i = start; i < end; i++) {
            ...
            if (isXHovered((int) mx, (int) my, px, ry)) {
                String name = Registries.BLOCK.getId(blocks.get(i)).getPath();
    ...
    // ── Helpers ────────────────────────────────────────────────────────────────
    private void tryAdd() {
        String raw = inputBox.getText().trim().toLowerCase();
        if (raw.isEmpty()) return;

        boolean added = module.addBlock(raw);
        if (added) {
            String shortName = raw.contains(":") ? raw.split(":")[1] : raw;
            feedback("§aAdded: §f" + shortName);
            inputBox.setText("");
